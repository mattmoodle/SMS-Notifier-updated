...
if($ADMIN->fulltree) {
	...
	$settings->add(new admin_setting_heading(
				'headerapi',
				'API settings',
				'Set up the API for SMS messaging'
			));
	...
	$settings->add(new admin_setting_heading(
				'headerrolesettings',
				'Role settings',
				'Define the users that can receive SMS'
			));
	...
	$roles = $DB->get_records('role', null, 'sortorder ASC');
	    ...
	    $settings->add(
	        new admin_setting_configmultiselect('block_sms_roleselection', 'Select roles allowed to receive SMS', 'Select multiple roles holding CTRL key. Here roles are shown with their shortnames.',
	            array_keys($defaults),
